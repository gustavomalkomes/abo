function [chosen_x_star, context] = expected_improvement_simple(...
...
options = optimoptions('fmincon', 'Display', 'none', 'MaxIterations', 20);
...
t = tic;
[chosen_x_star, ei_val, exitflag, output] = fmincon(fun,x0,[],[],[],[], ...
    zeros(d,1),ones(d,1), [], options);
time = toc(t);
ei_val = -ei_val;

fprintf(' %s >>> iter %d funcCount %d -- %f s\n', name, output.iterations, output.funcCount, time);
